...
class CDRL_Dataset_CutSwap(Dataset):
    def __init__(self, root_path=None, dataset=None, train_val=None, transforms_A=None, transforms_B=None, n_holes=None):
        ...
        for data in dataset.split(','):
            if data!='':
                ...
                self.total_path = self.total_path.replace("\\","/")
    ...
    def __getitem__(self, index):
        ...
        img_name = location.split('/')[-1]
        
        img_A = cv2.imread(location, cv2.IMREAD_COLOR)
        img_ori = img_A.copy()
        A2BB2A_path = location.split('/'+self.train_val+'/')[0]+'_A2B_B2A/'
        img_B = None
        if '/A/' in location:
        ...
        if not img_B.any():
            print("file_", index, len(self.files), index % len(self.files))
...
class CDRL_Dataset_test(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_A = cv2.imread(location, cv2.IMREAD_COLOR)
        img_B = cv2.imread(location.replace('/A/','/B/'), cv2.IMREAD_COLOR)
