...
def main():
    ...
    paths = [path.replace("\\","/") for idx, path 
        in enumerate(glob.glob('./pixel_img/'+opt.save_name+'/*'))]
    ...
    pred_path = [path.replace("\\","/") for idx, path 
        in enumerate(glob.glob('./pixel_img_morpho/'+opt.save_name+'/*'))]
